...
import { fetchDashboardStats, fetchDailyExpenses } from "@/lib/cached-queries";
...
export const fetchReferralDoctorsData = createCachedQueryFn({
  getKeyParts: (userId: string) => ["referral-doctors", userId],
  getTags: (userId: string) => [cacheTags.doctors(userId)],
  profile: "catalog",
  fn: async (userId: string): Promise<ReferralDoctorsData> => {
    ...
    const averageCommission =
      totalDoctors > 0
        ? Math.round(
            (rows.reduce((sum, row) => sum + row.commission, 0) / totalDoctors) *
              10,
          ) / 10
        : 0;
...
export const fetchBillsData = createCachedQueryFn({
  getKeyParts: (userId: string) => ["bills", userId],
  getTags: (userId: string) => [
    cacheTags.billingLedger(userId),
    cacheTags.patientReports(userId),
  ],
  profile: "live",
  fn: async (userId: string): Promise<BillsData> => {
...
export const fetchDueReportsData = cache(
  async (userId: string): Promise<DueReportsData> => {
    const { bills } = await fetchBillsData(userId);
    const dues = bills.filter((bill) => bill.balance > 0);
...
export const fetchReferralBusinessData = createCachedQueryFn({
  getKeyParts: (userId: string) => ["referral-business", userId],
  getTags: (userId: string) => [
    cacheTags.labInsights(userId),
    cacheTags.doctors(userId),
    cacheTags.patientReports(userId),
  ],
  profile: "live",
  fn: async (userId: string): Promise<ReferralBusinessData> => {
    const doctors = await prisma.doctor.findMany({
      where: { userId },
      select: {
        id: true,
...
export const fetchCaseViewReportsData = createCachedQueryFn({
  getKeyParts: (userId: string, dateFrom?: string, dateTo?: string) => {
    const range = resolveDateRange(dateFrom, dateTo);
    return ["case-view-reports", userId, range.dateFrom, range.dateTo];
  },
  getTags: (userId: string, _dateFrom?: string, _dateTo?: string) => [
    cacheTags.labInsights(userId),
    cacheTags.patientReports(userId),
  ],
  profile: "live",
...
export const fetchLabActivities = createCachedQueryFn({
  getKeyParts: (userId: string) => ["lab-activities", userId],
  getTags: (userId: string) => [
    cacheTags.labInsights(userId),
    cacheTags.patientReports(userId),
  ...
  profile: "live",
  fn: async (userId: string): Promise<LabActivity[]> => {
...
export const fetchLabNotifications = cache(
  async (userId: string): Promise<LabNotification[]> => {
    const todayKey = toDateKey(new Date());
    const [stats, dailyExpenses] = await Promise.all([
      fetchDashboardStats(userId),
      fetchDailyExpenses(userId, todayKey),
    ]);
    ...
    if (stats.pendingReports > 0) {
      notifications.push({
        id: "pending-reports",
        title: `${stats.pendingReports} report${stats.pendingReports === 1 ? "" : "s"} awaiting results`,
    ...
    if (stats.outstandingDues > 0) {
      notifications.push({
        id: "outstanding-dues",
        title: `₹${stats.outstandingDues.toLocaleString("en-IN")} outstanding across ${stats.duePatientCount} case${stats.duePatientCount === 1 ? "" : "s"}`,
    ...
    if (dailyExpenses.totalEntries === 0) {
      notifications.push({
        id: "no-expenses-today",
        title: "No expenses logged today",
        description:
          "Keep daily spending up to date for accurate business reports.",
...
export const fetchTestCategoriesWithCounts = createCachedQueryFn({
  getKeyParts: (userId: string) => ["test-categories-counts", userId],
  getTags: (userId: string) => [cacheTags.testCategories(userId)],
  profile: "catalog",
  fn: async (userId: string): Promise<TestCategoryRow[]> => {
