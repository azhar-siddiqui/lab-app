    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          side === "left"
            ? "left-0 group-data-[collapsible=offcanvas]:-left-(--infobar-width)"
            : "right-0 group-data-[collapsible=offcanvas]:-right-(--infobar-width)",
...
function InfobarGroupLabel({
  ...
  // 2. Use useRender to handle the element merging logic
  return useRender({
    render,
    defaultTagName: "div",
    props: {
...
function InfobarGroupAction({
  className,
  render, // Changed from asChild
  ...props
}: React.ComponentProps<"button"> & { render?: React.ReactElement }) {
  return useRender({
    render,
    defaultTagName: "button", // Replaces the "button" fallback
    props: {
...
function InfobarMenuButton({
  ...
  render?: React.ReactElement; // Changed from asChild
  isActive?: boolean;
  tooltip?: string | React.ComponentProps<typeof TooltipContent>;
} & VariantProps<typeof infobarMenuButtonVariants>) {
  const { isMobile, state } = useInfobar();

  // 1. Use useRender to create the button element
  ...
  return (
    <Tooltip>
      {/* 2. In Base UI, TooltipTrigger also uses the 'render' prop pattern */}
...
function InfobarMenuAction({
  className,
  render, // Replace asChild
  showOnHover = false,
  ...props
}: React.ComponentProps<"button"> & {
  render?: React.ReactElement; // Replace asChild
  showOnHover?: boolean;
}) {
  return useRender({
    render,
...
function InfobarMenuSubButton({
  ...
  return useRender({
    render,
    defaultTagName: "a", // Falls back to <a> if no render prop is provided
    props: {
